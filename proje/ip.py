...
class WorkerThread (threading.Thread):
    ...
    def Prewitt(self, header, patch, threshold):
    ...
    def Gaussian(self, header, patch, threshold):
        # convolve the patch with the matrix [[1,2,1],[2,4,2][1,2,1]]
        # read how the convolution is applied in discrete domain
        newMessage = [0] * self.patchsize * self.patchsize
        for i in range(1, self.patchsize-1):
            for j in range(1, self.patchsize-1):
        ...
        while(True):
            if self.inQueue.qsize() > 0:
                ...
                if str(message[0][0]) == "BinarizeFilter":
                    outMessage = self.Binarize(message[0][1], message[1],
                                                  128)
                if str(message[0][0]) == "GrayScale":
                    outMessage = self.convertGray(message[0][1], message[1])

                if str(message[0][0]) == "PrewittFilter":
                    outMessage = self.Prewitt(message[0][1], message[1], 128)
                if str(message[0][0]) == "GaussianFilter":
                    outMessage = self.Gaussian(message[0][1], message[1], 128)
                # self.pLock.acquire()
